  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 1) {
      if (/^batal$/i.test(text)) {
        ...
        return bot.sendMessage(chatId, `❌ *Proses Dibatalkan*\n\nExtrak nomor dibatalkan. Klik menu untuk memulai ulang.`, { parse_mode: "HTML", reply_markup: bot.getMainKeyboard() });
      }
    ...
    if (session.step === 2) {
      ...
      try {
        ...
        await bot.sendMessage(chatId, `✅ *EKSTRAK NOMOR SELESAI!</b>\n\n📄 File: \`${session.fileName}\`\n📊 Total Nomor: <b>${uniqueNumbers.length}*\n\n📝 Nomor-nomor Anda sudah di-extract dan disimpan ke file. Download file di bawah ini.\n\nSemoga membantu ya Kak! 😊`, { parse_mode: "HTML", reply_markup: bot.getMainKeyboard() });
        ...
        try { fs.unlinkSync(session.localPath); } catch {}
        delete sessions[userId];
        return bot.sendMessage(chatId, `❌ *Ekstrak Gagal*\n\nAda kesalahan saat memproses file.\nCoba ulangi atau hubungi owner.`, { parse_mode: "HTML", reply_markup: bot.getMainKeyboard() });
      }
    }
  });
}
