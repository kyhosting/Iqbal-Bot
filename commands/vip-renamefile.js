...
export default function (bot, db, saveDB) {
  ...
  bot.onText(/^⛓️ ʀᴇɴᴀᴍᴇ ꜰɪʟᴇ$/i, async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const role = bot.getRole(userId);

    if (!["owner", "admin", "vip", "trial"].includes(role)) {
      return bot.sendMessage(chatId, `◆ RENAME FILE\n\n▸ ❌ Akses Ditolak\n\nFitur ini khusus untuk VIP Kak\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
    }
    ...
    return await bot.sendMessage(chatId, `◆ RENAME FILE\n(Rename File)\n\n▸ Support Format:\n  • VCF (Contact)\n  • TXT (Text)\n  • XLSX (Excel)\n\n▸ Ubah nama file Anda\n\n▸ Ketik 'done' setelah selesai\n▸ Ketik 'batal' untuk membatalkan\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
  });
  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 1) {
      ...
      if (!msg.document) {
        return bot.sendMessage(chatId, "⚠️ *Kirim file dulu ya Kak* 😊", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }
      ...
      return bot.sendMessage(chatId, `📝 *Masukkan nama baru untuk file ya Kak*\n\nFile asli: \`${msg.document.file_name}\`\n\nKetik nama baru (tanpa ekstensi).\n\n✓ Ketik \`done\` setelah selesai\n✗ Ketik \`batal\` untuk batalkan`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
    }
    ...
    if (session.step === 2) {
      ...
      if (/^done$/i.test(text)) {
        fs.unlinkSync(session.file);
        delete sessions[userId];
        return bot.sendMessage(chatId, "❌ Nama file tidak boleh kosong Kak 😊\n\nCoba lagi ya!", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }

      let newName = text.trim().replace(/[^a-zA-Z0-9-_\s]/g, "_");
      ...
      try {
        ...
        bot.sendMessage(chatId, "⚠️ *Yah… ada masalah saat rename file* 😔\n\nCoba lagi ya Kak!", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }
