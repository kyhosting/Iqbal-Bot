...
export default function (bot, db, saveDB) {
  ...
  // GABUNG FILE - Support VCF dan TXT
  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 1) {
      ...
      if (!msg.document) {
        return bot.sendMessage(chatId, `◆ ɢᴀʙᴜɢ ꜰɪʟᴇ\n\n▸ ⚠️ Bukan File\n\nKirim file VCF atau TXT ya Kak\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }
      ...
      if (!isVcf && !isTxt) {
        return bot.sendMessage(chatId, `◆ ɢᴀʙᴜɢ ꜰɪʟᴇ\n\n▸ ⚠️ Tipe File Salah\n\nHanya VCF atau TXT yang didukung\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }

      const currentType = isVcf ? "vcf" : "txt";
    ...
    if (session.step === 2) {
      ...
      const ext = session.fileType === "vcf" ? ".vcf" : ".txt";
