    ...
    if (session.step === 2) {
      ...
      try {
        ...
        bot.sendDocument(chatId, filepath, {}, { filename }).then(() => {
          ...
          bot.sendMessage(chatId, "⚠️ *Yah… ada masalah saat kirim file* 😔", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
          try { fs.unlinkSync(filepath); } catch {}
        });
      } catch (err) {
        console.error("Gagal membuat file:", err);
        bot.sendMessage(chatId, "⚠️ *Yah… gagal buat file TXT* 😔", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }
