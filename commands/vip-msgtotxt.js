    ...
    if (session.step === 2) {
      ...
      try {
        ...
        bot.sendDocument(chatId, filepath, {}, { filename }).then(() => {
          ...
          bot.sendMessage(chatId, "⚠️ <b>Yah… ada masalah saat kirim file</b> 😔", { reply_markup: bot.getMainKeyboardUser(userId) });
          try { fs.unlinkSync(filepath); } catch {}
        });
      } catch (err) {
        console.error("Gagal membuat file:", err);
        bot.sendMessage(chatId, "⚠️ <b>Yah… gagal buat file TXT</b> 😔", { reply_markup: bot.getMainKeyboardUser(userId) });
      }
