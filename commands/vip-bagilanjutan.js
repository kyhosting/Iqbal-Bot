  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 2) {
      ...
      if (session.splitCounter === 1) {
        ...
        bot.sendMessage(chatId, `🪓 <b>Berapa jumlah file (bagian) yang mau dibuat ya Kak?</b>`,  { parse_mode: "HTML", reply_markup: bot.getMainKeyboardUser(userId) });
      }
      return;
    }
    ...
    if (session.step === 5) {
      ...
      try {
        ...
        console.error(err);
        bot.sendMessage(chatId, "⚠️ <b>Yah… ada masalah saat membagi file</b> 😔",  { parse_mode: "HTML", reply_markup: bot.getMainKeyboardUser(userId) });
      }
      return;
    }
