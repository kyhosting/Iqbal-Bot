  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 1) {
      ...
      if (!msg.document || !msg.document.file_name.endsWith(".vcf")) {
        return bot.sendMessage(chatId, "⚠️ *Harus file VCF ya Kak* 😊", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboardUser(userId) });
      }
    ...
    if (session.step === 6) {
      ...
      if (/^done$/i.test(text) || /^selesai$/i.test(text)) {
        delete sessions[userId];
        return bot.sendMessage(chatId, "✅ Semua proses selesai ya Kak! 😊", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboardUser(userId) });
      }
