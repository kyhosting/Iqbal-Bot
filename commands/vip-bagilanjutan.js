...
└─❖,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      ...
      session.newFileName = /^skip$/i.test(text) || !text ? session.originalName : text.replace(/[^a-zA-Z0-9-]/g, "");
      session.step = session.splitCounter === 1 ? 3 : 5;

      if (session.splitCounter === 1) {
        bot.sendMessage(chatId, 🔢 Masukkan angka awal penomoran kontak ya Kak\n\nContoh: 100 → "Nama-100", "Nama-101", dsb.,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      } else {
        bot.sendMessage(chatId, 🪓 Berapa jumlah file (bagian) yang mau dibuat ya Kak?,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      }
      return;
    }
...
└─❖,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      ...
      session.step = 4;
      bot.sendMessage(chatId, 🔢 Masukkan angka awal nama file ya Kak\n\nContoh: 1 → "nama-1.vcf", "nama-2.vcf", dsb.,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      return;
    }
...
└─❖,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      ...
      try {
        ...
        console.error(err);
        bot.sendMessage(chatId, "⚠️ Yah… ada masalah saat membagi file 😔",  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      }
      return;
    }
