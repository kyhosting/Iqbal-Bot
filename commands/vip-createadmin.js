...
└─❖,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
      ...
      try {
        ...
        bot.sendDocument(chatId, filepath).then(() => {
          trackMessage(userId, chatId, ✅ File ADMIN.vcf berhasil dibuat Kak! 🎉\n\n👤 Total admin: ${numbers.length}\n\nSemoga membantu ya! 😊,  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
          bot.incrementOperation(userId);
          fs.unlinkSync(filepath);
        }).catch((err) => {
          console.error("Gagal mengirim file:", err);
          bot.sendMessage(chatId, "⚠️ Yah… gagal kirim file 😔",  { parsemode: "Markdown", replymarkup: bot.getMainKeyboardUser(userId) });
          try { fs.unlinkSync(filepath); } catch {}
        });
      } catch (err) {
        console.error("Gagal membuat file:", err);
        bot.sendMessage(chatId, "⚠️ Yah… gagal buat file VCF 😔",  { parsemode: "Markdown", reply_markup: bot.getMainKeyboardUser(userId) });
      }
