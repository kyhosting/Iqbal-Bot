...
export default function (bot, db, saveDB) {
  ...
  bot.onText(/^⛓️ ʜɪᴛᴜɴɢ ꜰɪʟᴇ$/i, async (msg) => {
    ...
    if (!["owner", "admin", "vip", "trial"].includes(role)) {
      return bot.sendMessage(chatId, `◆ HITUNG FILE\n\n▸ ❌ Akses Ditolak\n\nFitur ini khusus untuk VIP Kak\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
    }
    ...
    return await bot.sendMessage(chatId, `◆ HITUNG FILE\n(Count Contacts)\n\n▸ Support Format:\n  • TXT (Text)\n  • VCF (Contact)\n\n▸ Hitung total kontak/nomor\n▸ Minimal 1 file\n\n▸ Ketik 'done' setelah selesai\n▸ Ketik 'batal' untuk membatalkan\n\n◆`, { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
  });
  ...
  bot.on("message", async (msg) => {
    ...
    if (session.step === 1) {
      ...
      if (!isTXT && !isVCF) {
        return bot.sendMessage(chatId, "⚠️ *Hanya support TXT atau VCF ya Kak* 😊", { parse_mode: "Markdown", reply_markup: bot.getMainKeyboard() });
      }
