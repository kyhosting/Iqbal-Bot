    ...
    if (session.step === 1) {
      ...
      try {
        ...
        if (total === 0) {
          ...
          delete sessions[userId];
          return bot.sendMessage(chatId, "⚠️ <b>Tidak ditemukan nama kontak di file ini Kak</b> 😔", { reply_markup: bot.getMainKeyboardUser(userId) });
        }
        ...
        await bot.sendMessage(chatId, hasil, { reply_markup: bot.getMainKeyboardUser(userId) });
        ...
        bot.sendMessage(chatId, "⚠️ <b>Yah… gagal baca file VCF</b> 😔\n\nPastikan formatnya benar ya!", { reply_markup: bot.getMainKeyboardUser(userId) });
        try {
